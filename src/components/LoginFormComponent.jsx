  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result?.perfil) {
        localStorage.setItem(
          "perfil",
          result.perfil.toLowerCase().trim()
        );
      }
      ...
      if (perfilUsuario === "admin") {
        ...
        toast.success(
          `Bem-vindo, ${result.nome || "Cliente"}!`
        );

        navigate("/home-cliente");
      } else if (perfilUsuario === "superadmin"){
        toast.success(
          `Bem-vindo, ${result.nome || "Super Admin"}!`
        );
    ...
    <div className=" bg-[#F5F5F3] flex flex-col items-center justify-center h-screen w-screen p-[32px]">
      ...
      <div className="w-[383px]">
        ...
        <form onSubmit={handleLogin}>
          ...
          <button className="bg-[#1E3A5F] text-white w-[383px] h-[39px] hover:bg-[#182e4b]" type="submit" >Entrar</button>
        </form>
